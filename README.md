...
・UI改善
-ボタンの角・色→カーソル後変化
-jsに直接記述したcssをstyle.cssに移動

・ポートフォリオ理解
-①render理解「画面を更新する関数」→「状態を元に画面更新」
-②filter理解「表示するタスクの条件を設定する処理」
-③localStorage理解「ブラウザにデータ保存」

まとめ
「このアプリでは、状態（todos）を元にrender関数で画面を更新しています。

タスクの表示はfilterを使って条件ごとに絞り込み、
localStorageを使うことでページ更新後もデータが保持されるようにしています。」
...
try{エラーが起きうる処理
}catch(引数e){エラー時に実行する処理}

throw new Error("エラー内容");

→コンソール上に「Uncaught Error: エラー内容」と表示される

「throwはエラーを発生させるだけで、画面表示は行いません。
表示するにはtry-catchで受け取り、別途UIに反映する必要があります。
